#!/bin/bash -xe
##################################################################################################################################################################
# Program       :  pgsql_vault_cleanup_script.tpl
...
if [ ${environment} != "production" ]; then
    ...
    vault delete corp-antmdb/aws/pgsql/nonprod/config/${dbinstance}
    vault delete corp-antmdb/aws/pgsql/nonprod/static-roles/${dbinstance}-master
    if [ $? -eq 0 ]; then
    ...
    echo -e "Deleting vault configs for ${dbinstance}"
    vault delete corp-antmdb/aws/pgsql/prod/config/${dbinstance}
    vault delete corp-antmdb/aws/pgsql/prod/static-roles/${dbinstance}-master
    if [ $? -eq 0 ]; then
        echo -e "Deletion of vault configs successful for ${dbinstance}!!"
    else
        echo -e "Error - Deleting vault configs of ${dbinstance}"
...
aws s3 cp /var/log s3://eh-dbaservices-log-${region}/pgsql/postscript/logs/${dbinstance}/ --exclude "*" --include "vault_cleanup_script.log" --recursive  --region ${aws_region}
